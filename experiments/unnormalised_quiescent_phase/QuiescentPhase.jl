...
function current_inversion_system(initial_current_data::Data, initial_params::Parameters, h::Int = 50)
    ...
    # Small epsilon around 0 value (half way to end state)
    # ϵ = 1e-5
    pϵ = 0.1 # 10% of total variation in the middle
    
    mid_current = (target_current + initial_current_data) / 2

    ϵ = (pϵ/2) * (initial_current_data - mid_current)

    start_current = mid_current + ϵ
    end_current = mid_current - ϵ
    ...
    for i in 1:h
        ...
        for (j::Int, curr::Datum) in pairs(start_current.d)
            sign = curr.val / abs(curr.val)

            new_d = curr - sign * (abs(end_current.d[j].val - curr.val) / h) * i
		...
		if i == 1
			M = maximum_current_for_params(cfg, previous_params)
		end
    ...
    gif(a, "simulated-inversion-fig-1-new.gif"; fps = 5)
end
...
function plot_for_start_params(params::Parameters, M::Float64)
	...
	println("First current: $(curr_vals[1])")
	println("Largest current: $(maximum(curr_vals))")
...
#i_params = Parameters(0.08, 3.5, 3.666)
