...
a1_range = range(-0.4, 0.4, length = 200)
...
manualDer = manualDer / maximum(abs.(manualDer))
calcDer = calcDer / maximum(abs.(calcDer))
residualVals = residualVals / maximum(abs.(residualVals))

p1 = plot(a1_range, calcDer, title = "Partial with respect to a1", label = "Analytic", xlabel = "a1", legend = false)
plot!(a1_range, manualDer, label = "Manual")
plot!(a1_range, residualVals, label = "Residual")
vline!([params.a1], label = "Expected")
savefig(p1, "partial-a1-residuals-fig-1.pdf")
...
a2_range = range(-2.6, 0.8, length = 200)
...
p2 = plot(a2_range, calcDer, title = "Partial with respect to a2", label = "Analytic", xlabel = "a2", legend = false)
plot!(a2_range, manualDer, label = "Manual")
plot!(a2_range, residualVals, label = "Residual")
vline!([params.a2], label = "Expected")
savefig(p2, "partial-a2-residuals-fig-1.pdf")
...
a3_range = range(1.0, 3.0, length = 200)
...
p3 = plot(a3_range, calcDer, title = "Partial with respect to alpha", label = "Analytic", xlabel = "alpha", legend = :outerright)
plot!(a3_range, manualDer, label = "Manual")
plot!(a3_range, residualVals, label = "Residual")
vline!([params.α], label = "Expected")
savefig(p3, "partial-alpha-residuals-fig-1.pdf")

p4 = plot(p1, p2, p3, layout = @layout [a b ; c])
savefig(p4, "conjoined.pdf")
